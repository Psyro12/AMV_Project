...
while($row = $result_cal->fetch_assoc()) {
    ...
    // Format dates for the modal text (e.g., "Tue 29 April")
    $checkInFmt = $start->format('D j F');
    $checkOutFmt = $end->format('D j F');
    ...
    for($date = clone $start; $date < $end; $date->modify('+1 day')) {
        ...
        $calendarData[$dateStr][] = [
            ...
            'status' => $row['status'],
            'type' => $colorType,
            'check_in_txt' => $checkInFmt,   // Added for modal
            'check_out_txt' => $checkOutFmt  // Added for modal
        ];
    }
}
...
<head>
    ...
    <style>
        ...
        .page-content {
            flex: 1;
            padding: 20px;
            overflow: hidden; /* Prevents scrollbars in content area */
            display: flex;
            flex-direction: column;
        }
        ...
        .calendar-grid-styled {
            display: grid;
            grid-template-columns: repeat(7, 1fr);
            flex: 1;              /* Added: Fills vertical space */
            grid-auto-rows: 1fr;  /* Added: Stretches rows to fill height evenly */
            overflow-y: auto;     /* Added: Scroll if content is too big */
        }
        .cal-cell {
        ...
        .cal-cell:hover { background-color: #f9f9f9; }
        
        .cal-cell.other-month { color: #eee; cursor: default; }
        .cal-cell.disabled-date { color: #ccc; background-color: #fafafa; cursor: not-allowed; }

        /* --- DYNAMIC STATUS COLORS (Calendar Grid) --- */
        ...
        .fs-xxs { font-size: 0.75rem; color: #666; font-weight: 500; }
        ...
        /* --- ✅ NEW MODAL STYLES (MATCHING IMAGE) ✅ --- */
        ...
        /* Modal Colors */
        .color-inhouse { background-color: #D3C855; } /* Gold/Olive */
        .color-booked { background-color: #F7EC73; color: #333 !important; } /* Yellow */
        .color-available { background-color: #ccc; } /* Grey */
...
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div class="page-content">
                <div class="page active" id="dashboard">
                    ...
                    <div class="charts-grid charts-grid-dashboard">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 1rem; flex: 0 1 320px; min-width: 300px;">
                            ...
                            <div class="chart-card" style="flex: 1; padding: 10px 20px; justify-content: center;">
                                <h3 class="chart-title fs-sm mb-2 text-center" style="text-align:center;">Monthly Bookings Breakdown</h3>
                                ...
                                <div class="d-flex justify-center g-3 mt-2">
                                    <div class="fs-xxs"><span class="legend-dot" style="background:#10B981"></span>Check-ins</div>
                                    <div class="fs-xxs"><span class="legend-dot" style="background:#F59E0B"></span>No-show</div>
                                    <div class="fs-xxs"><span class="legend-dot" style="background:#EF4444"></span>Cancelled</div>
                                </div>
                            </div>
                            ...
                            <div class="chart-card" style="flex: 0 0 auto; padding: 20px;">
                                <div class="progress-list">
                                    <div class="mb-2">
                                        <div class="d-flex-between fs-xxs mb-1"><span>Check-ins</span><span><?php echo $pctConfirmed; ?>%</span></div>
                                        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width: <?php echo $pctConfirmed; ?>%; background: #10B981;"></div></div>
                                    </div>
                                    <div class="mb-2">
                                        <div class="d-flex-between fs-xxs mb-1"><span>No-show</span><span><?php echo $pctPending; ?>%</span></div>
                                        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width: <?php echo $pctPending; ?>%; background: #F59E0B;"></div></div>
                                    </div>
                                    <div>
                                        <div class="d-flex-between fs-xxs mb-1"><span>Cancelled</span><span><?php echo $pctCancelled; ?>%</span></div>
                                        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width: <?php echo $pctCancelled; ?>%; background: #EF4444;"></div></div>
                                    </div>
                                </div>
                            </div>

                        </div>
                        <div class="chart-card" style="flex: 1 1 600px; min-width: 400px; display: flex; flex-direction: column;">
                            <div style="display:flex;align-items:center;justify-content:space-between; padding: 20px 20px 10px 20px;">
                            ...
                            <div class="chart-container" style="flex: 1; padding: 0 20px 20px 20px; position: relative; min-height: 0;">
                ...
                <div class="page" id="calendar" style="height: 100%; display: flex; flex-direction: column;">
                    ...
                    <div class="modal" id="calendarModal">
                        <div class="modal-content-calendar">
                            ...
                            <div class="modal-header px-4 pb-2">
                                <h3 id="calendarModalTitle">Room Status</h3>
                            </div>
                            <div class="modal-body px-4 pb-4" id="calendarModalBody">
                                </div>
                        </div>
                    </div>
                </div>

                <div class="page" id="guests" style="overflow-y: auto;"><h2 class="p-3">Guests Page</h2></div>
                <div class="page" id="bookings" style="overflow-y: auto;"><h2 class="p-3">Bookings Page</h2></div>
                <div class="page" id="food-ordered" style="overflow-y: auto;"><h2 class="p-3">Food Ordered Page</h2></div>
                <div class="page" id="transactions" style="overflow-y: auto;"><h2 class="p-3">Transactions Page</h2></div>
                <div class="page" id="settings" style="overflow-y: auto;"><h2 class="p-3">Settings Page</h2></div>
            </div>
        </main>
    </div>
    ...
    <script>
        ...
        // --- 3. CALENDAR LOGIC (UNIFORM 6 ROWS) ---
        ...
        function renderRealtimeCalendar() {
            ...
            // ✅ 1. PADDING (Previous Month) - Empty Text
            ...
            for(let i = 1; i <= daysInMonth; i++) {
                ...
                // Format readable date for modal title (e.g., "May 6")
                ...
                if (isPastDate) {
                    ...
                    cell.onclick = function() {
                        openRoomModal(readableDate, dayData);
                    };
                }
                grid.appendChild(cell);
            }

            // ✅ 3. FILL REMAINDER TO 42 CELLS
            const filledCount = firstDay + daysInMonth;
            const remaining = 42 - filledCount;
        ...
        // --- ✅ UPDATED MODAL LOGIC (MATCHES IMAGE) ---
        function openRoomModal(dateTitle, dayBookings) {
            document.getElementById('calendarModalTitle').innerText = `${dateTitle} - Rooms`;
            ...
            allRoomIds.forEach(roomId => {
                ...
                let colorClass = 'color-available';
                let statusText = 'Available';

                if (booking) {
                    if (booking.type === 'in_house') {
                        colorClass = 'color-inhouse'; // D3C855
                        statusText = `Occupied until ${booking.check_out_txt}`;
                    } else {
                        colorClass = 'color-booked'; // F7EC73
                        statusText = `Reserved from ${booking.check_in_txt} to ${booking.check_out_txt}`;
                    }
                }
                ...
                row.innerHTML = `
                    <div class="room-box ${colorClass}">${roomId}</div>
